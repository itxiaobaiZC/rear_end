<template>
    <div>
        ...
        <el-dialog title="轮播图信息" :visible.sync="dialogFormVisible" width="30%">
            <el-form label-width="80px" size="small">
                <el-upload  class="avatar-uploader" action="http://localhost:9090/file/upload" 
                    ...
                    >
                        <img v-if="form.pathurl" :src="form.pathurl" class="avatar">
        ...
        <el-table :data="tableData" style="padding: 5px 0;" border stripe :header-cell-class-name="tableHeadColor"
            ...
            <el-table-column prop="pathurl" label="图片" width="120px" >
                <template slot-scope="scope">
                    <img :src="(scope.row.pathurl)" alt="" style="width: 50px; height: 50px;">
                </template>
            </el-table-column>
    ...
    methods: {
        load() {
            this.request.get("/homepic/page", {
                ...
                this.imgUrl=res.records.pathurl
            })

            this.request.get("/homepic/findAll").then(res=>{
                this.roles=res.data
            })
        },
        ...
        delMul() {
            ...
            this.request.post("/homepic/deleteMul", ids).then(res => {
                if (res.code==='200') {
        ...
        save() {
            this.request.post("/homepic/save", this.form).then(res => {
                if (res.code==='200') {
        ...
        //删除用户
        handleDel(id) {
            this.request.delete("/homepic/delete/" + id).then(res => {
                if (res.code==='200') {
        ...
        handleAvatarSuccess(res){
            this.form.pathurl=res
        },
        changEnable(row){
            this.request.post("/homepic/updateById",row).then(res=>{
                if(res.code==='200'){
