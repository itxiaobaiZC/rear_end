    ...
    <div style="margin-left: 10%;">    
        <div class="pd-10" style="font-size: 20px; color: #3F5EFB; cursor: pointer;">{{ article.title }}</div>
            <div style=" margin-top: 10px">
                <i class="el-icon-user-solid"></i> <span >{{ article.title }}</span>
                <i class="el-icon-time" style="margin-left: 10px;"></i> <span >{{ article.releasetime }}</span>
            </div>
        <div>
            <mavon-editor 
                class="md"
            ...
            <div style="flex: 1; font-size: 14px; padding: 5px 0; line-height: 25px">
              ...
              <div style="display: flex; line-height: 20px; margin-top: 5px">
                ...
                <div style="text-align: right; flex: 1">
                  ...
                  <el-button type="text" style="color: red" @click="del(item.id)" v-if="user.id === item.userId ||user.name==='admin'">删除</el-button>
                </div>
              </div>
            </div>   <!--  内容-->
          </div>
              ...
              <div style="font-size: 14px; padding: 5px 0; line-height: 25px">
                ...
                <div style="display: flex; line-height: 20px; margin-top: 5px; padding-left: 5px">
                  ...
                  <div style="text-align: right; flex: 1">
                    ...
                    <el-button type="text" style="color: red" @click="del(subItem.id)" v-if="user.id === subItem.userId ||user.name==='admin'">删除</el-button>
                  </div>
                </div>
              </div>   <!--  内容-->
            </div>
...
export default {
    ...
    methods: {
        load() {
            ...
            this.request.get("/policy/findById/"+ id).then(res => {
                this.article = res.data
            })
        },
        loadComment() {
            this.request.get("/policycomment/tree/" + this.id).then(res => {
        ...
        save() {
            ...
            this.request.post("/policycomment/save", this.commentForm).then(res => {
                if (res.code === '200') {
        ...
        del(id) {
            this.request.delete("/policycomment/delete/" + id).then(res => {
                if (res.code === '200') {
