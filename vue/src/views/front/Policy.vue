    ...
    <div>
        ...
        <div style="margin: 10px 0">
            <div style="padding: 10px 0; border-bottom: 1px dashed #ccc" v-for="item in tableData" :key="item.id">
                <div class="pd-10" style="font-size: 20px; color: #3F5EFB; cursor: pointer" @click="$router.push('/front/policyDetail?id=' + item.id)">{{ item.title }}</div>
                <div style="font-size: 14px; margin-top: 10px">
                <i class="el-icon-user-solid"></i> <span >{{ item.title }}</span>
                <i class="el-icon-time" style="margin-left: 10px"></i> <span>{{ item.releasetime }}</span>
                </div>
            </div>
        </div>
        <!-- 分页栏 -->
