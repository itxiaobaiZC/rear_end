...
public class CommentController {
    ...
    public Result save(@RequestBody Comment comment){
        if (comment.getId() == null) { // 新增评论
            comment.setUserId(TokenUtils.getCurrentsysUser().getId());
        ...
        for (Comment origin : originList) {
            ...
            comments.forEach(comment -> {
                ...
                pComment.ifPresent((v -> {  // 找到父级评论的用户id和用户昵称，并设置给当前的回复对象
                    ...
                    comment.setPNickname(v.getUname());
                }));
            });
    ...
    public Page<Comment> findPage(@RequestParam Integer pageNum,
        ...
        return commentService.page(new Page<>(pageNum,pageSize),objectQueryWrapper);
    }
